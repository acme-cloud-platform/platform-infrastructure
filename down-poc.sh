#!/usr/bin/env bash
# down-poc.sh — tears down the poc environment only
set -euo pipefail
...
read -p "This will DESTROY the entire 'poc' environment. Type 'poc' to confirm: " CONFIRM
if [[ "$CONFIRM" != "poc" ]]; then
  echo "Confirmation did not match. Aborting, nothing destroyed."
  exit 1
fi

echo "=== Tearing down environment: poc ==="

# Same automatic stale-lock recovery as up-poc.sh — see that file for the
...
if aws eks describe-cluster --name acme-cloud-poc-eks --region us-east-1 >/dev/null 2>&1; then
  aws eks update-kubeconfig --name acme-cloud-poc-eks --region us-east-1 >/dev/null
  ...
  for i in $(seq 1 12); do
    ALB_COUNT=$(aws elbv2 describe-load-balancers --region us-east-1 \
      --query "length(LoadBalancers[?contains(LoadBalancerName, 'acmecloudpoc')])" \
      --output text 2>/dev/null || echo "0")
    if [[ "$ALB_COUNT" == "0" ]]; then
      echo "Confirmed: no ALB remaining for poc."
      break
    fi
  ...
  echo "(cluster acme-cloud-poc-eks not found — nothing to delete, likely already partially torn down)"
fi
...
# the SAME name on the next up-poc.sh run (AWS rejects reusing a name
...
aws secretsmanager delete-secret \
  --secret-id acme-cloud-poc-rds-credentials \
...
if aws secretsmanager describe-secret --secret-id acme-cloud-poc-rds-credentials --region us-east-1 >/dev/null 2>&1; then
  echo "WARNING: secret still exists, waiting a few seconds and re-checking..."
  sleep 5
  if aws secretsmanager describe-secret --secret-id acme-cloud-poc-rds-credentials --region us-east-1 >/dev/null 2>&1; then
    echo "WARNING: secret still not deleted. Check manually before running up-poc.sh again."
  else
...
echo ""
echo "--- removing stale kubectl context for acme-cloud-poc-eks ---"
ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
CONTEXT_NAME="arn:aws:eks:us-east-1:${ACCOUNT_ID}:cluster/acme-cloud-poc-eks"
