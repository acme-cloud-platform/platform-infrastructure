#!/usr/bin/env bash
# down-dev.sh — tears down the dev environment only
set -euo pipefail
export GODEBUG=http2client=0

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LIVE_DIR="$SCRIPT_DIR/live/dev"

read -p "This will DESTROY the entire 'dev' environment. Type 'dev' to confirm: " CONFIRM
if [[ "$CONFIRM" != "dev" ]]; then
  echo "Confirmation did not match. Aborting, nothing destroyed."
  exit 1
fi

echo "=== Tearing down environment: dev ==="

# Same automatic stale-lock recovery as up-dev.sh — see that file for the
...
if aws eks describe-cluster --name acme-cloud-dev-eks --region us-east-1 >/dev/null 2>&1; then
  aws eks update-kubeconfig --name acme-cloud-dev-eks --region us-east-1 >/dev/null
  ...
  for i in $(seq 1 12); do
    ALB_COUNT=$(aws elbv2 describe-load-balancers --region us-east-1 \
      --query "length(LoadBalancers[?contains(LoadBalancerName, 'acmeclouddev')])" \
      --output text 2>/dev/null || echo "0")
    if [[ "$ALB_COUNT" == "0" ]]; then
      echo "Confirmed: no ALB remaining for dev."
      break
    fi
  ...
  echo "(cluster acme-cloud-dev-eks not found — nothing to delete, likely already partially torn down)"
fi
...
# the SAME name on the next up-dev.sh run (AWS rejects reusing a name
...
aws secretsmanager delete-secret \
  --secret-id acme-cloud-dev-rds-credentials \
...
if aws secretsmanager describe-secret --secret-id acme-cloud-dev-rds-credentials --region us-east-1 >/dev/null 2>&1; then
  echo "WARNING: secret still exists, waiting a few seconds and re-checking..."
  sleep 5
  if aws secretsmanager describe-secret --secret-id acme-cloud-dev-rds-credentials --region us-east-1 >/dev/null 2>&1; then
    echo "WARNING: secret still not deleted. Check manually before running up-dev.sh again."
  else
...
echo ""
echo "--- removing stale kubectl context for acme-cloud-dev-eks ---"
ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
CONTEXT_NAME="arn:aws:eks:us-east-1:${ACCOUNT_ID}:cluster/acme-cloud-dev-eks"
