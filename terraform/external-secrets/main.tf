locals {
  ...
  eks_oidc_provider_arn = data.terraform_remote_state.alb_controller.outputs.eks_oidc_provider_arn
...
resource "kubernetes_manifest" "db_external_secret" {
  manifest = {
    ...
    spec = {
      ...
      dataFrom = [{
        extract = {
          key = data.terraform_remote_state.rds.outputs.db_secret_arn
        }
      }]
    }
  }
