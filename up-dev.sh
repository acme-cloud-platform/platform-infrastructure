#!/usr/bin/env bash
# up-dev.sh — brings up the dev environment only
set -euo pipefail
...
echo "=== Bringing up environment: dev ==="
...
echo ""
echo "=== Connecting kubectl to acme-cloud-dev-eks ==="
aws eks update-kubeconfig --name "acme-cloud-dev-eks" --region us-east-1

run ecr

# Note: down-dev.sh force-deletes the RDS Secrets Manager secret
...
# can safely recreate 'acme-cloud-dev-rds-credentials' with the same
